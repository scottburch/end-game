...
const howDataIsRetrievedPart = (speaker: Speaker) => of(undefined).pipe(
    // Endgame is a peer-to-peer network for data sharing.
    switchMap(videoPart(speaker, 'audio/how-endgame-works/intro.mp3', timer(5000))),
    // When a node requires some piece of data, it will ask the network for other nodes that have the data.
    switchMap(videoPart(speaker, 'audio/how-endgame-works/who-has-alice.mp3', playSvg('start', 'whoHasAlice').pipe(delay(5000)))),
    // Nodes that have the data will respond
    switchMap(videoPart(speaker, 'audio/how-endgame-works/i-do1.mp3', playSvg('', 'iDo1').pipe(delay(5000)))),
    ...
    switchMap(videoPart(speaker, 'audio/how-endgame-works/send-alice1.mp3', playSvg('', 'sendAlice').pipe(
        delay(1000),
        switchMap(() => playSvg('', 'sendAliceEnd')),
        delay(5000)
    ))),
    // As more data is requested, more nodes have the data providing reliability.
    switchMap(videoPart(speaker, 'audio/how-endgame-works/who-has-alice-2.mp3', playSvg('', 'whoHasAlice2').pipe(
        delay(1000),
    ...
    switchMap(videoPart(speaker, 'audio/how-endgame-works/get-bob.mp3', playSvg('', 'whoHasBob').pipe(
        delay(1000),
