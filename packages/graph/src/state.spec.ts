...
import {getEdge, getNode, putEdge, newGraphEdge, nodeId} from "./graph.js";
        ...
        firstValueFrom(startTestNet([[2], [3], [3], []]).pipe(
            switchMap(({node0, node1}) => of(undefined).pipe(
                ...
                switchMap(() => combineLatest([
                    getNode(node0, nodeId('thing1') , {}),
                    getNode(node1, nodeId('thing1') , {})
                ])),
                tap(([{node:n0}, {node:n1}]) => {
    ...
    it('should update edges according to state', () =>
        firstValueFrom(startTestNet([[2], [3], [3], []]).pipe(
            switchMap(({node0, node1}) => of(undefined).pipe(
                switchMap(() => graphNewAuth(node0, 'scott', 'pass')),
                switchMap(() => graphAuth(node0, 'scott', 'pass')),
                switchMap(() => putEdge(node0, newGraphEdge('e1', 'rel', nodeId('from') , nodeId('to') , {foo: 10}))),
                delay(6000),
                switchMap(() => putEdge(node0, newGraphEdge('e1', 'rel', nodeId('from') , nodeId('to') , {foo: 11}))),
                delay(6000),
                switchMap(() => combineLatest([
                    getEdge(node0, 'e1', {}),
                    getEdge(node1, 'e1', {})
