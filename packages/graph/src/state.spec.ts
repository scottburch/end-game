...
import {getEdge, getNodeInternal, putEdge, newEdge, asNodeId, asEdgeId} from "./graph.js";
        ...
        firstValueFrom(startTestNet([[2], [3], [3], []]).pipe(
            switchMap(({host0, host1}) => of(undefined).pipe(
                ...
                switchMap(() => combineLatest([
                    getNodeInternal(host0.graphs[0], asNodeId('thing0001') , {}),
                    getNodeInternal(host1.graphs[0], asNodeId('thing0001') , {})
                ])),
                tap(([{node:n0}, {node:n1}]) => {
    ...
    it('should update edges according to state', () =>
        firstValueFrom(startTestNet([[2], [3], [3], []]).pipe(
            switchMap(({host0, host1}) => of(undefined).pipe(
                switchMap(() => graphNewAuth(host0.graphs[0], 'scott', 'pass')),
                switchMap(() => graphAuth(host0.graphs[0], 'scott', 'pass')),
                switchMap(() => putEdge(host0.graphs[0], newEdge(asEdgeId('e1'), 'rel', asNodeId('from') , asNodeId('to') , {foo: 10}))),
                delay(6000),
                switchMap(() => putEdge(host0.graphs[0], newEdge(asEdgeId('e1'), 'rel', asNodeId('from') , asNodeId('to') , {foo: 11}))),
                delay(6000),
                switchMap(() => combineLatest([
