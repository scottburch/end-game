...
import {graphPutNode, newGraphNode} from "./graph.js";
...
describe('performance tests', () => {
    it('should continue to perform well with scale', () => {
         ...
         return firstValueFrom(getAGraph().pipe(
             ...
             switchMap(graph => graphPutNode(graph, newGraphNode('', 'person', {a:1, b:2, c:3, d: 4}))),
             tap(() => before = process.hrtime.bigint() - start),
             switchMap(({graph}) => range(1, 1_000).pipe(
                 mergeMap(() => graphPutNode(graph, newGraphNode('', 'person', {a:1, b:2, c:3, d: 4}))),
                 last(),
                 map(() => graph)
             )),
