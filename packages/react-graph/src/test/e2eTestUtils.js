...
export const compileBrowserTestCode = (src) => new Observable(subscriber => {
    let server;
    of({}).pipe(map(() => new WebpackDevServer({
        static: {
    ...
    }))), tap(s => server = s), switchMap(server => server.start()), tap(() => subscriber.next(undefined)), first()).subscribe();
    return () => {
        return server.stop();
    };
});
