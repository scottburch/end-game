...
import {graphOpen, putNode, LogLevel, newNode, newUid, nodeId} from "@end-game/graph";
...
export const startTestNode = (nodeNo: number, peers: number[] = [], basePort: number = 11110) =>
    graphOpen({graphId: `node-${nodeNo}`, logLevel: LogLevel.DEBUG}).pipe(
        switchMap(graph => levelStoreHandlers(graph)),
...
export const addThingNode = (graph: Graph, n: number, props: Props = {}) =>
    putNode(graph, newNode(nodeId(`thing${n}`), 'thing', {name: `thing${n}`, ...props}));
